...
import { Flame, Check, Zap, AlertCircle } from 'lucide-react';
...
export default function TaskCard({ task, onToggleComplete, onEdit, onArchive }: TaskCardProps) {
  ...
  const handleCheckboxChange = async () => {
    ...
    try {
      const result = await onToggleComplete(task, nextState);
      if (nextState && result?.xpEarned) {
        setFloatingXP(result.xpEarned);
        setTimeout(() => setFloatingXP(null), 1200);
      }
    } catch (err) {
      setCompleted(!nextState); // rollback on error
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div
      className={`system-panel rounded-xl p-4 transition-all duration-300 relative group overflow-hidden ${
        completed ? 'bg-surface/40 border-surface-border opacity-75' : 'hover:border-solo-blue/40'
      }`}
    >
      ...
      <div className="flex items-start gap-3 sm:gap-4">
        ...
        <button
          ...
          className={`w-7 h-7 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all duration-200 mt-0.5 ${
            completed
              ? 'bg-solo-cyan border-solo-cyan text-black shadow-[0_0_12px_rgba(34,211,238,0.5)]'
        ...
        <div className="flex-1 min-w-0">
          ...
          <h3
            className={`font-semibold text-sm sm:text-base text-text-primary transition-all duration-200 ${
